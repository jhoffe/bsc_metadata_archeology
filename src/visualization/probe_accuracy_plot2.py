...
def plot_probe_accuracy(df: pd.DataFrame, output_path: str, dataset_name: str) -> None:
    ...
    for suite_attr, suite_name in suite_names.items():
        ...
        df.loc[df["sample_index"].isin(indices), "suite"] = suite_name

    df.dropna(inplace=True)
    # Drop the first 512 duplicates
    df = df[~df.duplicated(subset=["sample_index", "suite", "epoch"], keep="last")]
    ...
    for i, suite in enumerate(suites):
        epoch = final[final["suite"] == suite]["epoch"]
        predictions = final[final["suite"] == suite]["prediction"]
